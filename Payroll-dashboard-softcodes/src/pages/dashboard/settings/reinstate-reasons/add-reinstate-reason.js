	...
	return (
		<>
			<Modal
				...
				title={isEdit ? 'Edit Reinstate Reason' : 'Add New Reinstate Reason'}
			>
						...
						<Formik
							...
							onSubmit={async ({ name, description, is_active }, resetForm) => {
								if (!isEdit) {
									...
									try {
										await editReinstateReason?.(data?.id, {
											name,
											description,
											is_active: is_active === 'active' || false,
										});
							...
							{({
								...
								<form onSubmit={handleSubmit}>
									<Input
										...
										type="text"
										label="Name*"
										placeholder="Enter Reinstate Reason"
										value={values.name}
									...
									>
										{isEdit ? 'Save Changes' : 'Add Reinstate Reason'}
									</Button>
								</form>
							)}
						</Formik>
